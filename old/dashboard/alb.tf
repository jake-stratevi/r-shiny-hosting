...
# ECS refuses to create a service against a target group that has no load
# balancer association. Our real rule points at the waker until it swaps, so
# the ECS group would otherwise be orphaned at create time. This rule attaches
# it to the listener on a condition nothing will ever match, purely to satisfy
# that requirement. It never serves traffic.
